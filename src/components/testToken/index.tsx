...
const network =
  "https://devnet.helius-rpc.com/?api-key=38caa145-8a0a-4499-a141-be31c8f4c784";
...
export const claimXSOL = async (publicKey: PublicKey, amount: number) => {
  try {
    // console.log(process.env.NEXT_PUBLIC_CONNECTION)
    const source_wallet = new PublicKey(
      "XdEqt8TDiG6HHxTCdo41FWwxt7qpthK5VWcXQWjthbS"
    );
    ...
    const sourceAccountAta = await getAssociatedTokenAddress(
      mint_address,
      source_wallet
    );
    const destinationAccountAta = await getAssociatedTokenAddress(
      mint_address,
      publicKey
    );
    ...
    const numberDecimals = (checkDecimals.value.data as ParsedAccountData)
    ...
    const sourceWalletKeypairString =
      "[209,174,191,23,162,17,90,120,119,10,162,129,102,112,254,55,34,0,251,151,0,136,148,17,139,179,182,35,175,245,175,98,7,216,102,67,96,114,252,224,248,112,137,241,50,183,197,158,137,134,177,28,46,169,248,74,87,68,83,145,107,153,146,229]";
